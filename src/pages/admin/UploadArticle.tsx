...
export default function UploadArticles() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!article.title || !article.sujet || !article.content) {
    ...
    setArticle({ title: "", sujet: "", desc: "", content: "", image: "" });
    setImagePreview(null);
  };

  return (
    <div className="max-w-3xl mx-auto p-6 bg-neutral-900 rounded-lg shadow-lg text-white">
      <h2 className="text-2xl font-semibold mb-4 text-center">Publier un article</h2>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col space-y-4">
        ...
        >
          <PlusCircle size={18} /> Publier l'article
        </button>
      </form>
    </div>
  );
